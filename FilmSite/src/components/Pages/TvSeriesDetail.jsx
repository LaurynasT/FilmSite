...
const TvSeriesDetails = () => {
  ...
  useEffect(() => {
    const loadTvSeriesData = async () => {
      try {
        const [tvData, tvDataCredits, tvSeasons, tvTrailer, tvSimilar] =
          await Promise.all([
    ...
    <div className="tvDetail">
      <div className="tvDetailwidth">
        <div className="background">
          ...
          <div className="tvDetail-container2">
            <h2>
              {tvDetail.name} ({tvDetail.first_air_date?.split("-")[0]})
            </h2>
        ...
        <div className="tvDetail-container3">
          ...
          <p>
            ...
            {tvDetail.networks.map((network) => network.name).join(", ")}
          </p>
          <p>
            <strong>Overview:</strong> {tvDetail.overview}
          </p>
          <p>
      ...
      <div className="seasons">
        ...
        <div className="seasons-scroll-wrapper">
          <button className="scroll-button seasons-left" onClick={scrollLeftE}>
            &#10094;
          </button>
          {episodes.length > 0 ? (
          ...
          <button
            className="scroll-button seasons-right"
            onClick={scrollRightE}
          >
            &#10095;
          </button>
        </div>
      ...
      <div className="cast-section">
        ...
        <div className="cast-scroll-wrapper">
          <button className="scroll-button cast-left" onClick={scrollLeft}>
            &#10094;
          </button>
          <div className="cast-scroll-container" ref={scrollRef}>
            {credits.cast.map((cast) => (
              <div
                key={cast.id}
                className="cast-card"
                onClick={() => handleNavigateToActor(cast.id)}
              >
                <img
                  src={
                    cast.profile_path
          ...
          <button className="scroll-button cast-right" onClick={scrollRight}>
            &#10095;
          </button>
        </div>
      </div>
            ...
            {Array.isArray(similar) && similar.length > 0 ? (
              similar.map((similars) => (
                <div
                  ...
                  key={similars.id}
                  onClick={() => handleNavigateToDetail(similars.id)}
                >
                  <img
                    src={
                      similars.poster_path
                        ? `${IMAGE_BASE_URL}${similars.poster_path}`
                        : "https://via.placeholder.com/120"
                    }
                    alt={similars.name}
                    className="similar-image"
                  />
                  <p style={{ color: "black" }}>{similars.name}</p>
                </div>
              ))
            ) : (
